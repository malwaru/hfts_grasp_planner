...
struct RelativeIndex {
    int x, y, z;
    RelativeIndex(int x_, int y_, int z_)
        : x(x_)
        , y(y_)
        , z(z_)
    {
...
struct Index {
    ...
    std::tuple<bool, Index> getNeighbor(const RelativeIndex& rel_index, const ssize_t* bounds) const
    {
        ...
        if (nx < 0 || nx >= bounds[0] || ny < 0 || ny >= bounds[1] || nz < 0 || nz >= bounds[2]) {
...
inline double compute_distance(const Index& a, const Index& b)
{
...
void computeMinSpanningtree(const np_b_array& input_arr, const std::vector<RelativeIndex>& neighbors, np_d_array& out_arr)
{
        ...
        for (size_t j = 0; j < input_arr.shape(1); ++j) {
            for (size_t k = 0; k < input_arr.shape(2); ++k) {
                ...
                if (!*input_arr.data(i, j, k)) {
                    ...
                    new_item.cell_state = CellInfo::CellState::InPQ;
    ...
    // std::cout << "Found " << pq.size() << " occupied cells, computing distances." << std::endl;
    ...
    while (!pq.empty()) {
        ...
        const PQElement& curr_elem = pq.top();
        CellInfo& current_cell = cells[curr_elem.id];
        current_cell.cell_state = CellInfo::CellState::Visited;
        ...
        for (auto& neigh : neighbors) {
            ...
            if (bvalid) {
                ...
                // we can skip cells for which we already have the minimal distance
                if (cells[nid].cell_state == CellInfo::CellState::Visited) {
                    continue;
                }
                // compute distance
    ...
    // std::cout << "Distance computation complete, copying data." << std::endl;
