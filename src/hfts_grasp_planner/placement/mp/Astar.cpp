...
struct AstarNode {
    ...
    // constructor
    AstarNode(NodePtr rnode, double dv = 0.0, std::shared_ptr<AstarNode> p = nullptr)
        : roadmap_node(rnode)
        , base_f_score(0.0)
        , base_parent(p)
    {
    ...
    : _env(penv)
{
    ...
    _roadmap = std::make_shared<Roadmap>(robot, _scene_interface, _scene_interface);
    // add start node
    Config start_config = _robot->GetActiveDOFValues();
...
void Astar::addGoal(const Goal& goal)
{
    ...
    _goal_nodes.insert(std::make_pair(gid, goal_node));
}
    ...
    for (unsigned int gid : goal_ids) {
        ...
        if (iter != _goals.end()) {
            ...
            if (goal_node_iter != _goal_nodes.end()) {
                goal_node_iter->second->is_goal = false;
