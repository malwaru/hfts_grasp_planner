...
def draw_volume(env, volume):
    return env.drawbox(0.5 * (volume[0] + volume[1]),
                       0.5 * (volume[1] - volume[0]),
...
def resolve_paths(problem_desc, yaml_file):
    ...
    for key in [
            'or_env', 'occ_file', 'sdf_file', 'urdf_file', 'target_obj_file',
            'grasp_file', 'robot_occtree', 'robot_occgrid',
            'reachability_path', 'robot_ball_desc', 'dmg_file',
            'gripper_information'
    ]:
        if key in problem_desc:
            problem_desc[key] = os.path.normpath(head + '/' +
                                                 problem_desc[key])
...
def show_solution(sol, target_obj):
    ...
    set_grasp(sol.manip, target_obj, inverse_transform(sol.grasp_tf),
              sol.grasp_config)
...
def tf_matrix_to_yaml_dict(m):
    ...
    """
    return {
        'pos': map(float, list(m[:3, 3])),
        'quat': map(float, list(orpy.quatFromRotationMatrix(m[:3, :3])))
    }
...
def store_placements(placements, grasp_set, waypoints, filename):
    ...
    if placements:
        ...
        grasp_ids = set([goal.grasp_id for goal in placements])
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        "Script to generate a collection of placements in a given scene.")
    parser.add_argument(
        'problem_desc',
        help="Path to a yaml file specifying what world, robot to use etc.",
        type=str)
    parser.add_argument('num_goals',
    ...
    try:
        ...
        try:
            ...
            rospy.logerr(
                "Could not load %s. Please provide an occupancy grid of the scene."
                % problem_desc['occ_file'])
        ...
        occ_target_volume = scene_occ.get_subset(placement_volume[0],
                                                 placement_volume[1])
        # create scene sdf
        dynamic_bodies = [
            body for body in env.GetBodies() if is_dynamic_body(body)
        ]
        scene_sdf = sdf_module.SceneSDF(env, [],
                                        excluded_bodies=dynamic_bodies)
        if os.path.exists(problem_desc['sdf_file']):
            ...
            rospy.logerr(
                "Could not load %s. Please provide a signed distance field of the scene."
                % problem_desc['sdf_file'])
            rospy.logerr("There is a script to create one!")
            sys.exit(0)
        # extract placement orientations
        target_object = env.GetKinBody(target_obj_name)
        orientations = plcmnt_orientations_mod.compute_placement_orientations(
            target_object)
        ...
        grasp_pose = orpy.matrixFromQuat(problem_desc["grasp_pose"][3:])
        grasp_pose[:3, 3] = problem_desc["grasp_pose"][:3]
        set_grasp(manip, target_object, inverse_transform(grasp_pose),
                  problem_desc['grasp_config'])
        gripper_info = afr_dmg_mod.load_gripper_info(
            problem_desc['gripper_information'], manip.GetName())
        ...
        manip_data = {}
        manip_data[manip.GetName(
        )] = afr_dmg_mod.MultiGraspAFRRobotBridge.ManipulatorData(
        ...
        robot_ball_approx = robot_sdf_module.RobotBallApproximation(
            robot, problem_desc['robot_ball_desc'])
        # build robot_octree
        try:
            now = time.time()
            robot_occgrid = robot_sdf_module.RobotOccupancyGrid.load(
                base_file_name=problem_desc['robot_occgrid'],
                robot=robot,
                link_names=link_names)
            rospy.logdebug("Loading robot occgrid took %fs" %
                           (time.time() - now))
        except IOError:
            now = time.time()
            robot_occgrid = robot_sdf_module.RobotOccupancyGrid(
                problem_desc['parameters']['occ_tree_cell_size'],
                robot=robot,
                link_names=link_names)
            robot_occgrid.save(problem_desc['robot_occgrid'])
            rospy.logdebug("Creating robot occgrid took %fs" %
                           (time.time() - now))
        # prepare robot data
        urdf_content = None
        ...
        # create object data
        obj_occgrid = kinbody_sdf_module.RigidBodyOccupancyGrid(
            problem_desc['parameters']['occ_tree_cell_size'],
            target_object.GetLinks()[0])
        obj_occgrid.setup_cuda_sdf_access(scene_sdf)
        object_data = afr_placement_mod.AFRRobotBridge.ObjectData(
            target_object, obj_occgrid)
        # create objective function
        now = time.time()
        if 'objective_fn' in problem_desc:
            if problem_desc['objective_fn'] == 'minimize_clearance':
                obj_fn = clearance_mod.ClearanceObjective(occ_target_volume,
                                                          obj_occgrid,
                                                          b_max=False)
                ...
                obj_fn = objectives_mod.DeepShelfObjective(target_object,
                                                           occ_target_volume,
                                                           obj_occgrid,
                                                           b_max=True)
        else:
        ...
        hierarchy = afr_placement_mod.AFRHierarchy([manip],
                                                   regions,
                                                   orientations,
                                                   so2_depth=4,
                                                   so2_branching=4)
        # create afr bridge
        afr_bridge = afr_dmg_mod.MultiGraspAFRRobotBridge(
            afr_hierarchy=hierarchy,
        ...
        # create goal sampler
        goal_sampler = simple_mcts_sampler_mod.SimpleMCTSPlacementSampler(
            hierarchy,
            afr_bridge,
            afr_bridge,
