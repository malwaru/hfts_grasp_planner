...
public:
  struct Grasp
  {
    ...
    std::string print() const
    {
      std::stringstream ss;
      ss << "id=" << id << " "
         << "x=" << pos.x << " y=" << pos.y << " z=" << pos.z << " qx=" << quat.x << " qy=" << quat.y
         << " qz=" << quat.z << " qw=" << quat.w;
      ss << " q=[";
  ...
  struct Solution
  {
    ...
    double cost;
    Solution() : goal_id(0), path(nullptr), cost(INFINITY)
    {
    }

    Solution(unsigned int goal, WaypointPathPtr ipath, double icost) : goal_id(goal), path(ipath), cost(icost)
    {
    }
  };
