...
class GoalSamplingStatsRecorder(object):
    """
        ...
        register_new_goal(..) function, which stores the following information:
            - runtime when the solution was found (relative to start time)
    ...
    def register_new_goal(self, solution):
        ...
        self._stats.append((time.time() - self._start_time,  # runtime
...
class PlacementMotionStatsRecorder(object):
    ...
    def register_new_solution(self, sol, num_grasps=1):
        ...
        self._stats.append((time.time() - self._start_time, sol.sample_num,
                            sol.objective_value, x, y, z, ex, ey, ez, sol.grasp_id, num_grasps))
    ...
    def save_stats(self, file_name):
        ...
        with open(file_name, 'w') as the_file:
            the_file.write(
                'runtime,#samples,objective,x,y,z,ex,ey,ez,grasp_id,#grasps\n')
            for stat in self._stats:
                the_file.write(str(stat).replace('(', '').replace(')', '') + '\n')
