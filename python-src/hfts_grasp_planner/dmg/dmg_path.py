...
def matrixToVectorPose(matrix):
    ...
    vec = np.append(tf.transformations.translation_from_matrix(matrix), q)
    return vec
...
class DMGPushPath():
    ...
    def translationPushPoses(self, translation, object_to_fixed_gripper, current_fingertip_contacts):
        ...
        T_start = np.array([np.append(x_axis, 0), np.append(y_axis, 0), np.append(z_axis, 0), np.append(push_point, 1)]).T
        #transformation matrix containing the pose of the gripper pusher at the end (after translation)
        T_end = np.array([np.append(x_axis, 0), np.append(y_axis, 0), np.append(z_axis, 0), np.append(push_point - translation, 1)]).T
    ...
    def rotationPushPoses(self, rotation, object_to_fixed_gripper, current_grasp_pose, current_fingertip_contacts):
        ...
        push_point_to_object = np.array([np.append(x_axis, 0), np.append(y_axis, 0), np.append(z_axis, 0), np.array([0, 0, 0, 1])]).T
        push_point_to_object[0, 3] = push_point[0]
        push_point_to_object[1, 3] = push_point[1]
        push_point_to_object[2, 3] = push_point[2]
    ...
    def convert_path(self, inhand_path, initial_grasp_pose, fingertip_contacts):
        """
            ...
            Returns
            -------
            pusher_poses, a list of np.array of shape (7,) - where each pose is represented as 
                (tx, ty, tz, qx, qy, qz, qw) with t being the translation and q quaternion
            rotation_centers, a list of np.arrays
        """
        ...
        current_fingertip_contacts = fingertip_contacts
        ...
        self._gripper_to_fingertip[2, 3] = -np.linalg.norm(fingertip_middle_point - current_object_to_gripper[0:3, 3:4].T[0])
        ...
        if len(rotations) > 0:
            ...
            if abs(r) > 0.00001:
                ...
                pose1, pose2 = self.rotationPushPoses(r, current_object_to_gripper, current_grasp_pose, current_fingertip_contacts)
                if not pose1 is None:
                    # pass
                    pusher_poses.append(matrixToVectorPose(pose1))
                    pusher_poses.append(matrixToVectorPose(pose2))
        ...
        for idx, t in enumerate(translations):
            ...
            if abs(r) > 0.00001:
                ...
                if not pose1 is None:
                    pusher_poses.append(matrixToVectorPose(pose1))
                    pusher_poses.append(matrixToVectorPose(pose2))
                #the fingertip contacts to not vary, but the pose does (around the x axis)
                x_axis = np.array([1, 0, 0])
                current_object_to_fingertip = np.dot(self._gripper_to_fingertip, current_object_to_gripper)
                current_object_to_fingertip = np.dot(axisAngle(x_axis, r), current_object_to_fingertip)
                current_object_to_gripper = np.dot(self._fingertip_to_gripper, current_object_to_fingertip)
